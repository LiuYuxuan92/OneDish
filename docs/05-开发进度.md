# 简家厨 - 开发进度记录

## 2026-02-21 内容深度扩展（方向A）

### 已完成功能
- **后端数据库**: 新增 `stage`/`first_intro`/`key_nutrients`/`scene_tags`/`texture_level` 字段到 recipes 表
- **新建 baby_stages 表**: 6个发育阶段完整指南数据
- **菜谱扩充**: 从100道扩充到140道，新增40道分阶段辅食菜谱（6-8m到24-36m各阶段）
- **BabyStageService**: REST API `/api/v1/baby-stages` 支持按阶段/月龄查询
- **前端辅食体系**: BabyStageScreen（阶段列表）+ StageDetailScreen（阶段详情+食谱筛选）
- **StageGuideCard**: 可折叠展开的阶段喂养指南组件
- **首页集成**: HomeScreen 今日辅食建议卡片
- **菜谱列表集成**: RecipeListScreen 辅食体系入口 Banner
- **React Query Hooks**: useAllBabyStages / useBabyStageByAge / useStageRecipes

### 文件变更
- `backend/src/database/migrations/20260221000001_add_stage_fields_to_recipes.ts`
- `backend/src/database/migrations/20260221000002_create_baby_stages_table.ts`
- `backend/src/database/seeds/007_baby_stages.ts`
- `backend/src/database/seeds/008_baby_recipes.ts`
- `backend/src/services/baby-stage.service.ts`
- `backend/src/controllers/baby-stage.controller.ts`
- `backend/src/routes/baby-stage.routes.ts`
- `backend/src/index.ts` (注册新路由)
- `frontend/src/types/index.ts` (新增 BabyStageGuide, BabyStageKey)
- `frontend/src/api/babyStages.ts`
- `frontend/src/hooks/useBabyStages.ts`
- `frontend/src/components/recipe/StageGuideCard.tsx`
- `frontend/src/screens/recipe/BabyStageScreen.tsx`
- `frontend/src/screens/recipe/StageDetailScreen.tsx`
- `frontend/src/screens/home/HomeScreen.tsx`
- `frontend/src/screens/recipe/RecipeListScreen.tsx`
- `frontend/src/navigation/RecipeNavigator.tsx`

---

## 2026-02-20 开发进度

### Phase 2-4 架构改进（全部完成）

详见 `docs/07-架构改进方案.md`，共完成 6 个改进方案：

| Phase | 内容 | 状态 |
|-------|------|------|
| 2A | 同步烹饪时间线 (`POST /recipes/:id/timeline`) | ✅ |
| 2B | 搜索结果保存到我的菜谱 (`user_recipes` 表) | ✅ |
| 3A | 智能周计划（月龄过滤 + 营养均衡 + 食材复用） | ✅ |
| 3B | 食材生命周期闭环（购物→库存→扣减→临期推荐） | ✅ |
| 4A | 离线优先架构（PersistQueryClientProvider + NetInfo） | ✅ |
| 4B | 转换缓存持久化（内存 Map → SQLite transform_cache 表） | ✅ |

### 烹饪模式（Cooking Mode）实现

**设计文档：** `docs/plans/2026-02-20-cooking-mode-design.md`
**实施计划：** `docs/plans/2026-02-20-cooking-mode-plan.md`

#### 新增文件

**前端：**
- `frontend/src/screens/recipe/CookingModeScreen.tsx` — 全屏沉浸式烹饪主屏幕
- `frontend/src/hooks/useCookingSession.ts` — 烹饪会话状态管理 hook（含语音播报、AsyncStorage 持久化）
- `frontend/src/components/cooking/StepCard.tsx` — 步骤卡片（颜色编码：共用/大人/宝宝/分叉）
- `frontend/src/components/cooking/StepTimer.tsx` — 步骤倒计时组件
- `frontend/src/components/cooking/CrossLineAlert.tsx` — 大人/宝宝切换主动提醒 Modal
- `frontend/src/components/cooking/BabyStepCard.tsx` — 宝宝专属步骤卡片（月龄适配 + 过敏提示）
- `frontend/src/constants/babyAgeRules.ts` — 月龄规则配置（质地建议 + 过敏食材规则）

#### 修改文件

**前端：**
- `frontend/src/types/index.ts` — RecipeStackParamList / HomeStackParamList / PlanStackParamList 添加 `CookingMode` 路由
- `frontend/src/navigation/RecipeNavigator.tsx` — 注册 CookingModeScreen
- `frontend/src/navigation/HomeNavigator.tsx` — 注册 CookingModeScreen
- `frontend/src/navigation/PlanNavigator.tsx` — 注册 CookingModeScreen
- `frontend/src/screens/recipe/RecipeDetailScreen.tsx` — 同步烹饪 Tab 底部添加"开始烹饪"按钮

#### 功能说明

**Phase A — 核心框架：**
- 全屏沉浸式界面，隐藏 Tab Bar
- `expo-keep-awake` 屏幕常亮（进入时激活，退出时释放）
- 烹饪进度自动保存到 AsyncStorage，意外退出可恢复
- 步骤卡片按类型颜色编码（共用=青、大人=橙、宝宝=粉、分叉=黄）
- 并行步骤小卡片（`parallel_with` 字段关联）
- 步骤倒计时（开始/暂停/重置）

**Phase B — 体验增强：**
- `expo-speech` 语音播报，步骤切换自动朗读（rate: 0.85，zh-CN）
- 🔊/🔇 开关，随时静音
- 左右滑动切换步骤，双击重新朗读（react-native-gesture-handler）
- 大人→宝宝切换时 `expo-haptics` 震动 + 主动提醒 Modal

**Phase C — 月龄精细化：**
- 月龄规则配置表（6月泥状 → 8月糊状 → 10月细碎 → 12月小块 → 18月正常）
- 宝宝步骤显示月龄适配说明和质地建议
- 首次出现高风险过敏食材时弹出提示（花生/虾/蟹/蛋白/蜂蜜/坚果/牛奶）
- "为什么这样做？"展开科学依据 Modal

#### 依赖安装

```bash
npx expo install expo-speech
npx expo install expo-haptics
```

#### 使用方式

1. 打开任意菜谱详情页
2. 切换到"同步烹饪"Tab
3. 点击底部"🍳 开始烹饪（同步模式）"
4. 进入全屏烹饪模式，按步骤操作

---

## 2026-02-15 开发进度

### 新增功能: 联网搜索菜谱

实现了三种搜索来源的智能切换:
1. **本地数据库** - 已有80+道菜谱
2. **天行数据API** - 联网扩展 (免费100次/天) ✅ 已配置
3. **MiniMax AI** - 智能菜谱推荐 ✅ 已配置

#### 新增文件

**后端:**
- `backend/src/adapters/search.adapter.ts` - 搜索适配器接口
- `backend/src/adapters/local.adapter.ts` - 本地数据库适配器
- `backend/src/adapters/tianxing.adapter.ts` - 天行数据API适配器
- `backend/src/adapters/ai.adapter.ts` - MiniMax AI适配器
- `backend/src/services/search.service.ts` - 统一搜索服务
- `backend/src/routes/search.routes.ts` - 搜索API路由

**前端:**
- `frontend/src/api/search.ts` - 搜索API封装
- `frontend/src/hooks/useSearch.ts` - 搜索Hook

#### 修改文件

- `backend/src/index.ts` - 注册搜索路由
- `backend/.env` - 添加API配置
- `frontend/src/types/index.ts` - 添加source字段
- `frontend/src/components/recipe/RecipeCard.tsx` - 显示来源标签
- `frontend/src/screens/recipe/RecipeListScreen.tsx` - 添加联网搜索按钮

#### 使用方式

1. 在菜谱列表页面，输入搜索词
2. 点击"联网搜索"按钮切换到联网模式
3. 搜索结果会显示来源标签: 本地/联网/AI推荐

---

## 2026-01-21 开发进度

### 已完成功能

#### 1. 菜谱管理模块
- ✅ 15个新菜谱数据入库
- ✅ 菜谱详情展示（大人版/宝宝版）
- ✅ 菜谱搜索功能
- ✅ 菜谱分类筛选

#### 2. 餐食计划模块
- ✅ 一周餐食计划展示
- ✅ 按日期设置餐食计划
- ✅ 餐食计划删除功能

#### 3. 购物清单模块
- ✅ 生成购物清单（基于餐食计划）
- ✅ 购物清单按区域分组显示（超市区/蔬果区/调料区/其他）
- ✅ 全选/取消全选功能
- ✅ 删除清单项功能
- ✅ 手动添加清单项功能
- ✅ 单个项目勾选功能

---

### 2026-01-21 修复问题

#### 问题1: 单个购物清单项无法勾选

**问题描述**：
- 点击单个购物清单项无法切换选中状态
- 只能通过"全选"按钮操作
- 前端有点击事件触发（黄色高亮），但UI不更新

**根本原因**：
后端 `updateListItem` 方法存在两个bug：

1. **JSON解析问题**：
   - 从数据库读取的 `list.items` 是JSON字符串
   - 代码直接类型转换未解析
   - 导致 `items[area]` 访问失败

2. **JSON序列化问题**：
   - 更新数据库时未使用 `JSON.stringify(items)`
   - 对象被存为 `"[object Object]"` 字符串
   - 后续读取导致JSON解析失败

**修复方案**：

```typescript
// 修复1: 解析items从JSON字符串到对象
const items = typeof list.items === 'string'
  ? JSON.parse(list.items)
  : list.items;

// 修复2: 序列化items为JSON字符串存储
.update({ items: JSON.stringify(items) })
```

---

#### 问题2: 获取购物清单API返回500错误

**问题描述**：
- 打开今日清单页面直接失败
- GET `/shopping-lists?start_date=2026-01-21` 返回500错误

**根本原因**：
- 数据库中存在corrupted数据 (`items: '[object Object]'`)
- `JSON.parse('[object Object]')` 抛出异常
- 未处理的异常导致API崩溃

**修复方案**：
在所有解析items的方法中添加try-catch错误处理：

```typescript
let parsedItems;
try {
  parsedItems = typeof list.items === 'string'
    ? JSON.parse(list.items)
    : list.items;
} catch (e) {
  console.error('[Backend] Failed to parse items for list:', list.id, e);
  parsedItems = {
    超市区: [],
    蔬果区: [],
    调料区: [],
    其他: [],
  };
}
```

**修复的方法** (共6个)：
1. `getShoppingLists` - 获取历史购物清单
2. `getShoppingListById` - 获取单个清单详情
3. `addRecipeToShoppingList` - 添加菜谱到清单
4. `removeListItem` - 删除清单项
5. `addListItem` - 手动添加清单项
6. `toggleAllItems` - 全选/取消全选

---

### 待修复问题

#### 1. 添加物品弹框自动关闭问题
**描述**：点击物品名称或数量的输入框时，弹框会自动关闭，无法输入
**优先级**：高
**相关文件**：`frontend/src/screens/plan/ShoppingListScreen.tsx`

#### 2. 删除按钮可能无法点击
**描述**：用户反馈删除功能可能无效
**优先级**：中

---

### 技术栈

**后端**：
- Node.js + Express
- SQLite + Knex.js
- TypeScript
- tsx watch (开发模式)

**前端**：
- React Native Web
- TanStack Query (React Query)
- Axios
- TypeScript

**开发工具**：
- Metro bundler (前端)
- tsx watch (后端)

---

### 开发信息

- 后端服务运行在端口 3000
- 前端开发服务器运行在端口 8085 (Web)
- 数据库文件位于 `backend/dev.sqlite3`
- 所有API路径以 `/api/v1` 开头

### 启动命令

```bash
# 后端
cd backend && npm run dev

# 前端 (Web)
cd frontend && npx expo start --web --port 8085
```
