...
export function MembershipScreen({ navigation }: Props) {
  ...
  const featureGroups = useMemo(() => {
    ...
    return CATEGORY_ORDER
      ...
      .map(key => ({ key, title: CATEGORY_LABELS[key] || '会员权益', items: grouped.get(key) || [] }));
  }, [matrixQuery.data, summaryQuery.data?.feature_matrix]);

  const isLoading = authLoading || productsQuery.isLoading || matrixQuery.isLoading || (isAuthenticated && summaryQuery.isLoading);
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.Colors.background.secondary }]} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={[styles.heroCard, activeEntitlement ? styles.heroCardActive : null]}>
          <Text style={styles.heroEyebrow}>{activeEntitlement ? 'MEMBERSHIP ACTIVE' : 'ONE ACCOUNT · MULTI PLATFORM'}</Text>
          <Text style={styles.heroTitle}>{activeEntitlement ? '成长会员已开通' : '成长会员在 App 解锁完整体验'}</Text>
          <Text style={styles.heroSubtitle}>
            {activeEntitlement
              ? `当前有效期至 ${formatDate(activeEntitlement.ends_at)}`
              : '同一会员账号，小程序可轻量使用，App 可使用完整家庭管理与复盘能力。'}
          </Text>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              Alert.alert(
                ...
                selectedProduct ? `当前支付链路仍在接入中。推荐档位：¥${selectedProduct.price_yuan}` : '当前支付链路仍在接入中。'
              );
            }}
          >
            <Text style={styles.heroButtonText}>{activeEntitlement ? '续费会员' : selectedProduct ? `¥${selectedProduct.price_yuan}/月` : '查看方案'}</Text>
          </TouchableOpacity>
        </View>
        ...
        {isAuthenticated && quotaCards.length ? (
          <View style={[styles.section, { backgroundColor: theme.Colors.background.primary }]}> 
            ...
            <View style={styles.summaryRow}>
              {quotaCards.map(item => (
                <View key={item.feature_code} style={[styles.summaryCard, { backgroundColor: theme.Colors.background.tertiary }]}> 
                  <Text style={[styles.summaryValue, { color: theme.Colors.primary.dark }]}>{item.remaining_quota}/{item.total_quota}</Text>
          ...
          {featureGroups.map(group => (
            <View key={group.key} style={styles.groupBlock}>
              ...
              {group.items.map(item => (
                <View key={item.feature_code} style={[styles.featureCard, { backgroundColor: item.app_only_value ? theme.Colors.background.tertiary : theme.Colors.background.secondary }]}> 
                  <View style={styles.featureHeader}>
                    <Text style={[styles.featureTitle, { color: theme.Colors.text.primary }]}>{item.display_name}</Text>
                    <View style={[styles.featureTag, { backgroundColor: item.access_policy === 'member_quota' ? Colors.primary.main : Colors.secondary.main }]}>
                      <Text style={styles.featureTagText}>{item.access_policy === 'member_quota' ? '次数权益' : item.app_only_value ? 'App 完整体验' : '会员权益'}</Text>
                    </View>
                  </View>
                  <Text style={[styles.featureDesc, { color: theme.Colors.text.secondary }]}>{item.description}</Text>
                  <Text style={[styles.featureExtra, { color: theme.Colors.text.tertiary }]}>{item.upsell_copy || '同一会员账号可在小程序和 App 同步权益。'}</Text>
                </View>
              ))}
            </View>
          ))}
          ...
          <View style={[styles.section, { backgroundColor: theme.Colors.background.primary }]}> 
            ...
            {summaryQuery.data.recent_orders.slice(0, 3).map(order => (
              <View key={order.id} style={styles.orderRow}>
                <Text style={[styles.orderTitle, { color: theme.Colors.text.primary }]}>{order.metadata?.product_name || order.product_code}</Text>
                <Text style={[styles.orderDesc, { color: theme.Colors.text.secondary }]}>{order.status === 'paid' ? '已支付' : '待支付'} · ¥{order.amount_yuan}</Text>
              </View>
            ))}
          </View>
        ) : null}
        ...
        {showDevTools ? (
          <View style={[styles.section, styles.devSection, { backgroundColor: theme.Colors.background.primary }]}> 
            ...
            <Text style={[styles.devDesc, { color: theme.Colors.text.secondary }]}>仅开发环境显示，方便你自己反复验证开会员、发 AI 包、重置额度和重测购买链路。</Text>
            <View style={styles.devButtonRow}>
              <TouchableOpacity style={[styles.devButton, styles.devButtonPrimary]} disabled={devActionLoading} onPress={() => grantDevProduct('growth_monthly_1990', '月卡')}>
                <Text style={styles.devButtonPrimaryText}>{devActionLoading ? '处理中...' : '一键开月卡'}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.devButton} disabled={devActionLoading} onPress={() => grantDevProduct('growth_quarterly_4900', '季卡')}>
                <Text style={styles.devButtonText}>一键开季卡</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.devButtonRow}>
              <TouchableOpacity style={styles.devButton} disabled={devActionLoading} onPress={() => grantDevProduct('ai_baby_pack_20_990', '宝宝 AI 加油包')}>
                <Text style={styles.devButtonText}>发宝宝 AI 包</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.devButton} disabled={devActionLoading} onPress={() => runDevAction(() => billingApi.devResetQuotas(['ai_baby_recipe', 'weekly_plan_from_prompt', 'smart_recommendation']).then((res) => res.data || res), '所有 AI 测试额度已重置。')}>
            ...
            <TouchableOpacity
              ...
              onPress={() => Alert.alert('清空测试权益？', '会将当前账号的会员与测试额度都置为失效，便于重新走购买流程。', [
                { text: '取消', style: 'cancel' },
                { text: '确认清空', style: 'destructive', onPress: () => runDevAction(() => billingApi.devClearBenefits().then((res) => res.data || res), '当前账号的测试权益已清空。') },
              ])}
            >
        ...
        <View style={[styles.section, { backgroundColor: theme.Colors.background.primary }]}> 
          ...
          <View style={styles.infoBlock}>
            ...
            <Text style={[styles.infoText, { color: theme.Colors.text.secondary }]}>成长会员绑定账号，同一账号登录小程序和 App 都能享受对应权益。</Text>
          </View>
          <View style={styles.infoBlock}>
            <Text style={[styles.infoTitle, { color: theme.Colors.text.primary }]}>为什么 App 功能更多？</Text>
            <Text style={[styles.infoText, { color: theme.Colors.text.secondary }]}>App 更适合库存管理、家庭协作、周复盘和烹饪模式，小程序则更适合快速决策。</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  summaryCard: { minWidth: '30%', flexGrow: 1, borderRadius: BorderRadius.lg, padding: Spacing.md },
  ...
  groupTitle: { fontSize: Typography.fontSize.sm, fontWeight: Typography.fontWeight.semibold, marginBottom: Spacing.sm, textTransform: 'uppercase' },
  featureCard: { borderRadius: BorderRadius.lg, padding: Spacing.md, marginBottom: Spacing.sm },
  ...
  devSection: { borderWidth: 1, borderColor: '#CFE1FF', borderStyle: 'dashed' },
  devDesc: { fontSize: Typography.fontSize.sm, lineHeight: 20, marginBottom: Spacing.md },
  devButtonRow: { flexDirection: 'row', gap: Spacing.sm, marginBottom: Spacing.sm },
  devButton: { flex: 1, borderRadius: 999, paddingVertical: Spacing.sm, paddingHorizontal: Spacing.md, backgroundColor: '#F4F8FF', alignItems: 'center', justifyContent: 'center' },
  devButtonPrimary: { backgroundColor: Colors.primary.main },
  devButtonDanger: { backgroundColor: '#FFF2F0', marginTop: Spacing.xs },
  ...
  devButtonDangerText: { color: '#C0392B', fontSize: Typography.fontSize.sm, fontWeight: Typography.fontWeight.bold },
